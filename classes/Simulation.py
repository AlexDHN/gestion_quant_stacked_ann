...
class Simulation:
    def __init__(self, **kwargs) -> None:
        ...
        if "path" in kwargs:
            self.Ann = Ann.load(kwargs["path"])  # It is assumed that the characteristics of the loaded Ann correspond to the input data
        else:
            self.Ann = Ann(
                input_size = len(self.tab[0]),  
                output_size = 1, 
            )
                ...
                for batch in iterator:
                    ...
                    with torch.set_grad_enabled(phase=='train'):
                        ...
                        if self.l2_regularization:
                            loss += self.l2_lambda*sum(p.pow(2.0).sum()for p in self.Ann.parameters())
